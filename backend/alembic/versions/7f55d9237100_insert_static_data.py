...
Revision ID: 7f55d9237100
Revises: 8f2af354fce6
...
from exercise_db.models.models import (
    Exercise,
    ExerciseSet,
    Muscle,
    MuscleGroup,
...
def insert_records():  # exercise_table
    bind = op.get_bind()
    ...
    sit_ups = Exercise(name="Sit-up", muscle_primary=abdominals.id, is_compound=False)
    ...
    exercises = [
        ...
        ("Clean", glutes.id, quads.id, True),
        ("Clean and Jerk", glutes.id, quads.id, True),
        ("Snatch", glutes.id, quads.id, True),
        ...
        ("Power Clean", glutes.id, quads.id, True),
        ("Dumbbell Clean and Jerk", glutes.id, quads.id, True),
        ...
        ("Bulgarian Split Squat", quads.id, glutes.id, True),
    ...
    upper_a = WorkoutTemplate(program_id=new_program.id, day_of_week=2, label="Upper A")
    lower_a = WorkoutTemplate(program_id=new_program.id, day_of_week=3, label="Lower A")
    upper_b = WorkoutTemplate(program_id=new_program.id, day_of_week=5, label="Upper B")
    lower_b = WorkoutTemplate(program_id=new_program.id, day_of_week=6, label="Lower B")
    ...
    template_exercises = [
        ...
        TemplateExercise(order=1, workout_template_id=lower_a.id, exercise_id=squat.id),
        TemplateExercise(order=2, workout_template_id=lower_a.id, exercise_id=sl_dl.id),
        TemplateExercise(
        ...
        TemplateExercise(order=1, workout_template_id=upper_b.id, exercise_id=cg_bp.id),
        TemplateExercise(order=2, workout_template_id=upper_b.id, exercise_id=ohp.id),
        TemplateExercise(
        ...
        TemplateExercise(order=2, workout_template_id=lower_b.id, exercise_id=fr_sq.id),
        TemplateExercise(
