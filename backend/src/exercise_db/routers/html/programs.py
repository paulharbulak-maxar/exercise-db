...
from exercise_db.models.models import Muscle, Program, ProgramType, WorkoutTemplate
from exercise_db.routers import templates
from exercise_db.routers.html.workout_templates import router as template_router
...
def create_program_html(
    ...
    with Session(engine) as session:
        ...
        return RedirectResponse(
            router.url_path_for("get_program_html", program_id=program.id),
            status_code=status.HTTP_303_SEE_OTHER,
        )


@router.get("/")
def get_programs_html(request: Request):
    with Session(engine) as session:
...
def get_program_html(request: Request, program_id: int):
    with Session(engine) as session:
...
def delete_program_html(program_id: int):
    with Session(engine) as session:
        program = session.exec(select(Program).where(Program.id == program_id)).one()
        ...
        return RedirectResponse(
            router.url_path_for("get_programs_html"),
            status_code=status.HTTP_303_SEE_OTHER,
        )
...
def create_workout_template_html(
    program_id: int,
    ...
    with Session(engine) as session:
        ...
        return RedirectResponse(
            template_router.url_path_for(
                "get_workout_template_html", template_id=workout_template.id
            ),
            status_code=status.HTTP_303_SEE_OTHER,
        )
