...
from exercise_db.routers.html.workouts import router as workout_router
...
def update_workout_template_html(
    template_id: int,
...
def get_workout_template_html(request: Request, template_id: int):
    with Session(engine) as session:
        workout_template = session.exec(
            select(WorkoutTemplate).where(WorkoutTemplate.id == template_id)
        ).one()
...
def create_template_exercise_html(
...
def create_workout_html(
    ...
    with Session(engine) as session:
        ...
        return RedirectResponse(
            workout_router.url_path_for("get_workout_html", workout_id=workout.id),
            status_code=status.HTTP_303_SEE_OTHER,
        )
